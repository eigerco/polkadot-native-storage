...
pub use self::{
    errors::Error,
    network::{current_network, set_current_network, Network},
    payload::{DelegatedAddress, Payload},
    protocol::Protocol,
};
use crate::ActorID;
...
fn bls_zero_addr_bytes() -> [u8; BLS_PUB_LEN] {
    let bz_addr = Network::Mainnet.parse_address(
        "f3yaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaby2smx7a",
    );
    if let Ok(Address {
        payload: Payload::BLS(pubkey),
        ..
    }) = bz_addr
    {
        pubkey
...
#[derive(Copy, Clone, Debug, Hash, PartialEq, Eq, PartialOrd, Ord, TypeInfo, Encode, Decode)]
...
impl Serialize for Address {
        ...
        S: Serializer,
    {
        ...
        Serialize::serialize(&address_bytes, s)
    }
}
...
impl<'de> Deserialize<'de> for Address {
    ...
    where
        D: Deserializer<'de>,
    {
        let bz: Cow<'de, [u8]> = Deserialize::deserialize(deserializer)?;
